...
import { createCsvContent, GET_TAB_SHOP_RANK_CAPTURE, getCaptureFileName, type GetLatestShopRankPageResponse, type ShopRankCapture, type ShopRankRequestSeen } from '../../src/shared/shopRank';
...
const capture = ref<ShopRankCapture | null>(null);
const noticeType = ref<NoticeType>('info');
const noticeMessage = ref('请先打开罗盘店铺榜单页，并刷新页面触发接口请求。');
// 保留最近一次 background 返回的原始响应，供诊断面板展示，方便定位捕获失败原因。
const lastResponse = ref<GetLatestShopRankPageResponse | null>(null);
...
const requestSeenText = computed(() => formatRequestSeen(lastResponse.value?.shopRankRequestSeen ?? null));
...
async function loadLatestCapture () {
    ...
    try {
        ...
        // 统一向 background 查询当前 tab 的最新捕获，不再直接问顶层 frame，
        ...
        if (!response.bridgeReady) {
            ...
            noticeType.value = 'info';
            noticeMessage.value = `已注入 ${frameCount} 个 frame，请刷新罗盘页面或点击页面筛选触发接口后，再点刷新捕获。`;
            return;
        }
        ...
        if (!response.pageReady) {
            ...
            noticeMessage.value = '采集 bridge 已就绪，但页面 patch 脚本未上报。可能是 MAIN world 注入失败，请刷新罗盘页面后再试。';
            return;
        }
        ...
        if (response.fetchPatched === false) {
            ...
            noticeType.value = 'warning';
            noticeMessage.value = '页面 fetch 未成功 patch（可能被罗盘框架覆盖）。已尝试在 DOMContentLoaded 重打补丁，请刷新页面后再点刷新捕获。';
            return;
        }

        if (response.shopRankRequestSeen) {
            // background 探测到请求时已经尝试了重取。重取成功的话 capture 应该非空，
            // 不会走到这个分支；走到这里说明重取也失败了。
            const time = new Date(response.shopRankRequestSeen.at).toLocaleTimeString('zh-CN');
            noticeType.value = 'warning';
            noticeMessage.value = `已检测到 shop_rank 请求（${time}），但背景重取未成功。请确认已登录罗盘，或尝试刷新页面后重试。`;
            return;
        }

        // 脚本就绪、fetch 已 patch，但还没看到请求，提示用户去触发接口。
        noticeType.value = 'info';
        noticeMessage.value = '采集脚本已就绪，请刷新罗盘页面或点击页面筛选按钮触发 shop_rank 接口后，再点刷新捕获。';
    } catch (error) {
        capture.value = null;
        noticeType.value = 'warning';
...
function assertCompassTab (url: string | undefined) {
    if (!url) {
        throw new Error('当前标签页地址不可读，请切到罗盘店铺榜单页后再试。');
    }

    const parsedUrl = new URL(url);

    if (parsedUrl.hostname !== COMPASS_HOST) {
        throw new Error('请先切到 compass.jinritemai.com 的罗盘店铺榜单页，再点击刷新捕获。');
    }
}

async function requestLatestCapture (tabId: number): Promise<GetLatestShopRankPageResponse> {
    const response = await browser.runtime.sendMessage({ type: GET_TAB_SHOP_RANK_CAPTURE, tabId }) as GetLatestShopRankPageResponse | undefined;
...
function updateNoticeByResponse (response: GetLatestShopRankPageResponse) {
...
function updateNoticeByCapture (nextCapture: ShopRankCapture | null) {
    if (!nextCapture) {
        noticeType.value = 'info';
        noticeMessage.value = '采集脚本已加载，但还没有捕获到店铺榜单接口。请刷新罗盘页面或点击页面筛选按钮触发接口后，再点刷新捕获。';
        return;
    }
...
function downloadCsv () {
    ...
    link.download = getCaptureFileName(capture.value);
    link.click();
...
function getFriendlyErrorMessage (error: unknown): string {
    ...
    return message || '读取当前页数据失败，请刷新罗盘页面后重试。';
}
...
function formatRequestSeen (value: ShopRankRequestSeen | null): string {
    if (!value) {
        return '无';
    }
  ...
  <main class="popup-page">
    ...
    <a-collapse class="debug-collapse" :bordered="false" ghost :default-active-key="[]">
      <a-collapse-panel key="debug" header="诊断信息">
        ...
        <div class="summary-row">
          <span class="summary-label">shop_rank 请求</span>
